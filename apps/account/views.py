...
from apps.account.forms import AuthorizationForm, RegistrationForm, ForgotForm, UserProfileForm
...
def registration_confirm(request, auth_key):
    if UserProfile.objects.filter(auth_key=auth_key).exists():
        ...
        if user.is_active == True:
            ...
            user_profile.save()
            messages.success(request, _(u'Ваша учетная запись активирована<br />Теперь вы можете авторизироваться на сайте'))
    ###
    return HttpResponseRedirect(reverse(DEFAULT_REDIRECT))
    ...
    if (user.is_authenticated() == False):
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                c = RequestContext(request, data)
                html = t.render(c)
                msg = EmailMessage(_(u'Studiogen - восстановление пароля'), html, EMAIL_FROM, [send_data['email']])
                msg.content_subtype = "html"
                msg.send()
                messages.success(request, _(u'На указанный емаил выслано письмо с новым паролем'))
                return HttpResponseRedirect(reverse(DEFAULT_REDIRECT))
        else:
            form = ForgotForm()
    else:
            ...
            if form.is_valid():
                ...
                if user is not None:
                    if user.is_active:
                        ...
                        messages.error(request, _(u'Ваш профиль заблокирован'))
                else:
                    messages.error(request, _(u'Не правильно указан логин или пароль'))
            else:
                messages.error(request, _(u'Ошибка заполнения полей'))
        else:
